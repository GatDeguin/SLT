...
def generate_multistream_dataset(root: Path, spec: SyntheticDatasetSpec) -> SyntheticDatasetPaths:
    ...
    pose_dir = root / "pose"
    for directory in (face_dir, hand_left_dir, hand_right_dir, pose_dir):
    ...
    for vid_idx, video_id in enumerate(video_ids):
        ...
        _write_pose(pose_dir / f"{video_id}.npz", spec.frames_per_video, spec.pose_landmarks, seed=spec.seed + vid_idx)
