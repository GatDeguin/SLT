...
def _write_csv(path: Path, rows: Iterable[PredictionItem]) -> None:
    entries = list(rows)
    include_gloss = any(item.gloss_prediction for item in entries)
    ...
    with NamedTemporaryFile(
        ...
        for row in entries:
            ...
            output = [row.video_id, row.prediction, row.reference, latency]
            if include_gloss:
                output.extend([row.gloss_prediction or "", row.gloss_reference or ""])
