...
- `visualize_keypoints_viewer.py`: renderiza video, keypoints y subtítulos para
  ...
  Al usar `--videos-dir`/`--keypoints-dir` imprime un resumen con coincidencias, faltantes
  y archivos huérfanos para validar el dataset antes de entrenar.
  ...
  y restablece los ajustes iniciales con `R`. Presiona `N` para saltar al siguiente clip sin
  cerrar la ventana o `P` para volver al anterior. Usa `--start-paused`, `--no-subtitles` y
